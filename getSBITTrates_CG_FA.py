...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("No Files given Usage: python getSBITTrates.py <CG_data_file.csv> <FA_data_file.csv>")
        exit(1)
        #print("Using default data.csv if it is present")
        #CG_data_path ="data.csv"
        #FA_data_path ="data.csv"
    else:
        CG_data_path = sys.argv[1]

        if len(sys.argv)>=3:
            FA_data_path = sys.argv[2]

    master_path='./csv_files/SBI_REFERENCE_RATES_USD.csv'

    if len(sys.argv)>=3:
        update_sbi_rates(data_path= FA_data_path, master_path= master_path,date_col=["Date Acquired","Date Sold"],typ='FA')
    update_sbi_rates(data_path= CG_data_path, master_path= master_path,date_col=["Date Acquired","Date Sold"],typ='CG')